## Course Project 1


## Plot 1
library(data.table)
data <- data.table(read.csv("household_power_consumption.txt", header = TRUE, sep = ";"))
subSetData <- data[data$Date %in% c("1/2/2007","2/2/2007") ,]

hist(Global_active_power, col="red", main="Global Active Power", xlab="Global Active Power (kilowatts)")


## Plot 2
library(data.table)
data <- data.table(read.csv("household_power_consumption.txt", header = TRUE, sep = ";"))
subSetData <- data[data$Date %in% c("1/2/2007","2/2/2007") ,]
DateandTime <- strptime(paste(subSetData$Date, subSetData$Time, sep=" "), "%d/%m/%Y %H:%M:%S") 

plot(DateandTime, subSetData$Global_active_power, type="l", ylab="Global Active Power (kilowatts)")

## Plot 3
library(data.table)
data <- data.table(read.csv("household_power_consumption.txt", header = TRUE, sep = ";"))
subSetData <- data[data$Date %in% c("1/2/2007","2/2/2007") ,]
DateandTime <- strptime(paste(subSetData$Date, subSetData$Time, sep=" "), "%d/%m/%Y %H:%M:%S") 

plot(DateandTime, subSetData$Sub_metering_1, type = 'l', ylab = "Energy sub metering")
lines(DateandTime, subSetData$Sub_metering_2, type="l", col="red")
lines(DateandTime, subSetData$Sub_metering_3, type="l", col="blue")
legend("topright", c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), lty=1, lwd=2.5, col=c("black", "red", "blue"))

## Plot 4
library(data.table)
data <- data.table(read.csv("household_power_consumption.txt", header = TRUE, sep = ";"))
subSetData <- data[data$Date %in% c("1/2/2007","2/2/2007") ,]
DateandTime <- strptime(paste(subSetData$Date, subSetData$Time, sep=" "), "%d/%m/%Y %H:%M:%S") 

par(mfrow = c(2, 2)) 
plot(DateandTime, subSetData$Global_active_power, type = 'l', ylab = "Global Active Power", cex=0.2)
plot(DateandTime, subSetData$Voltage, type = "l", xlab = "DateandTime", ylab = "Voltage")
plot(DateandTime, subSetData$Sub_metering_1, type = 'l', ylab = "Energy sub metering")
lines(DateandTime, subSetData$Sub_metering_2, type="l", col="red")
lines(DateandTime, subSetData$Sub_metering_3, type="l", col="blue")
legend("topright", c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), lty=1, lwd=2.5, col=c("black", "red", "blue"))
plot(DateandTime, subSetData$Global_reactive_power, type="l", xlab="datetime", ylab="Global_reactive_power")

